...
//----------------------------------------------------------
//----------------------------------------------------------
double f(double x, double *array, size_t array_size) {
  // sum1 : y_i^(x) ln(y_i)
  double sum1 = 0;
...
double derivative_f(double x, double *array, size_t array_size) {
  double h = 1e-5;
  return (f(x + h, array, array_size) - f(x, array, array_size)) / h;
}
...
int main(int argc, char **argv) {
  ...
  while (fabs(x2 - x1) >= delta && fabs(f(x1, y, n)) >= epsilon &&
         derivative_f(x1, y, n) != 0) {
    x2 = x1;
    x1 = x1 - f(x1, y, n) / derivative_f(x1, y, n);

    printf(
        "Iteration: %d, x1 = %.6f, x2 = %.6f, f(x2) = %.6f, |x1 - x2| = %.6f\n",
        counter, x1, x2, f(x2, y, n), fabs(x2 - x1));
