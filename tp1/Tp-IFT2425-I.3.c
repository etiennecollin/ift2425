...
double derivative_f(double x) {
    ...
    double sum1 = 0;  // sum y_i^x * (log(yi))^2
    double sum2 = 0;  // sum y_i^x
    double sum3 = 0;  // sum y_i^x * log(yi)

    for (int i = 0; i < array_size; i++) {
        sum1 += pow(array[i], x) * pow(log(array[i]), 2);
        sum2 += pow(array[i], x);
        sum3 += pow(array[i], x) * log(array[i]);
    }

    return (sum1 * sum2 - pow(sum3, 2)) / pow(sum2, 2) + 1 / pow(x, 2);
}
