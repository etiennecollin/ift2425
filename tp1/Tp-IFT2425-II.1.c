...
bool is_divergent(double real_part, double imaginary_part) {
  ...
  if (modulus > 2) {
    return true; // The sequence diverges to infinity
  }
...
int main(int argc, char **argv) {
  ...
  if (flag_graph) {
    ...
    win_ppicture =
        fabrique_window(nomfen_ppicture, 10, 10, width, length, zoom);
    x_ppicture = cree_Ximage(Graph2D, zoom, length, width);

    // Sauvegarde
    SaveImagePgm((char *)"", (char *)"FractalMandelbrot", Graph2D, length,
                 width);
