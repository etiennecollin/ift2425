...
/************************************************************************/
Window fabrique_window(char *nom_fen, int x, int y, int width, int height,
                       int zoom) {
  Window win;
...
XImage *cree_Ximage(float **mat, int z, int length, int width) {
      ...
      for (col = 0; col < wdth; col = col + z) {
        ...
        for (zoom_lig = 0; zoom_lig < z; zoom_lig++)
          for (zoom_col = 0; zoom_col < z; zoom_col++) {
            dat[((lig + zoom_lig) * wdth * 4) + ((4 * (col + zoom_col)) + 0)] =
                pix;
            dat[((lig + zoom_lig) * wdth * 4) + ((4 * (col + zoom_col)) + 1)] =
                pix;
            dat[((lig + zoom_lig) * wdth * 4) + ((4 * (col + zoom_col)) + 2)] =
                pix;
            dat[((lig + zoom_lig) * wdth * 4) + ((4 * (col + zoom_col)) + 3)] =
                pix;
          }
      }
  } /*--------------------------------------------------------*/
  ...
  imageX = XCreateImage(display, visual, depth, ZPixmap, 0, (char *)dat, wdth,
                        lgth, 16, wdth * 4);
  return (imageX);
}
...
/****************************************************************************/
XImage *cree_XimageWithMvt(float **matImg, float **vctMvt, int z, int length,
                           int width) {
  int i;
  int lgth, wdth, lig, col, zoom_col, zoom_lig;
  float sum[3];
    ...
    for (lig = 0; lig < lgth; lig = lig + z)
      for (col = 0; col < wdth; col = col + z) {
        ...
        for (zoom_lig = 0; zoom_lig < z; zoom_lig++)
          for (zoom_col = 0; zoom_col < z; zoom_col++) {
            if (pixM) {
              dat[((lig + zoom_lig) * wdth * 4) +
                  ((4 * (col + zoom_col)) + 0)] = 0;
              dat[((lig + zoom_lig) * wdth * 4) +
                  ((4 * (col + zoom_col)) + 1)] = 255;
              dat[((lig + zoom_lig) * wdth * 4) +
                  ((4 * (col + zoom_col)) + 2)] = 0;
            }
          }
...
//----------------------------------------------------------
void free_fmatrix_1d(float *pmat) { free(pmat); }
...
//----------------------------------------------------------
void DrawLine(float **Img, int colbeg, int rowbeg, int colend, int rowend,
              int wdth, int lgth) {
  int k;
  float cord, pente;
  float incrow, inccol;
  ...
  if (fabs(diffrow) >= fabs(diffcol)) {
    ...
    if (rowend >= rowbeg)
      for (k = rowbeg; k <= rowend; k++) {
        if ((k > 0) && (cord > 0) && (k < lgth) && (cord < wdth))
          Img[k][(int)cord] = 255;
        cord += pente;
      }
  ...
  if (fabs(diffcol) > fabs(diffrow)) {
    ...
    if (colend >= colbeg)
      for (k = colbeg; k <= colend; k++) {
        if ((k > 0) && (cord > 0) && (k < wdth) && (cord < lgth))
          Img[(int)cord][k] = 255;
        cord += pente;
      }
...
// PutArrowOnImg
//----------------------------------------------------------
void PutArrowOnImg(float **Img, int lgth, int wdth, int PosX, int PosY,
                   float IncPosX, float IncPosY) {
  double Theta;
  double PX, PY;
...
//----------------------------------------------------------
void ConvertVelocityFieldInAroowField(float ***SeqImgOptFlot, float ***Vx,
                                      float ***Vy, int lgth, int wdth, int nbit,
                                      int INC) {
  int i, j, k;
  ...
  for (k = 0; k < nbit; k++)
    for (i = INC; i < (lgth - INC); i += INC)
      for (j = INC; j < (wdth - INC); j += INC)
        PutArrowOnImg(SeqImgOptFlot[k], lgth, wdth, j, i, Vx[k][i][j],
                      Vy[k][i][j]);
}
...
int main(int argc, char **argv) {
  ...
  // Ix, Iy, It sont des tableaux 2D de même dimension que l'image qui ont été
  // allouées VxM, VyM  Vecteur Vx & Vy moyenné aussi OptFl_Vx, OptFl_Vy qui
  // contiendra le flot optique
  //
  //-----------------------------------------------------------
  printf("\n\n Jacobi Iterations :\n");
  ...
  // Sauvegarde
  printf("\n");
  SaveImagePpm("Optical_Flot_WithImg", Img2, SeqImgOptFlot[NBITER - 1], length,
               width);
  ...
  //>Window creation
  if (open_display() < 0)
    printf("Graphical Session Error");
  sprintf(nomfen_ppicture1, "Image Sequence");
  sprintf(nomfen_ppicture2, "Optical Flow");
  sprintf(nomfen_ppicture3, "Image + Optical Flow");
  win_ppicture1 =
      fabrique_window(nomfen_ppicture1, 10, 10, width, length, zoom);
  win_ppicture2 = fabrique_window(nomfen_ppicture2, 12 + (zoom * width), 10,
                                  width, length, zoom);
  win_ppicture3 = fabrique_window(nomfen_ppicture3, 12 + (zoom * width), 10,
                                  width, length, zoom);
  ...
  for (k = 0; k < NBITER; k++) {
    ...
    x_ppicture3 =
        cree_XimageWithMvt(Img2, SeqImgOptFlot[k], zoom, length, width);
  ...
  // Liberation Memoire
  if (SeqImgOptFlot)
    free_fmatrix_3d(SeqImgOptFlot, NBITER);
  if (OptFl_Vx)
    free_fmatrix_3d(OptFl_Vx, NBITER);
  if (OptFl_Vy)
    free_fmatrix_3d(OptFl_Vy, NBITER);
  if (Ix)
    free_fmatrix_2d(Ix);
  if (Iy)
    free_fmatrix_2d(Iy);
