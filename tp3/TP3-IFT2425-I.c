...
float calculateIntegral(float a, float b, int nb_intervals) {
    float h = (b - a) / nb_intervals;
    float integral = 0.0;
...
float* calculate_array_subintegrals(float a, float b, int nb_intervals) {
    float* array = new float[nb_intervals];
    float h = (b - a) / nb_intervals;
    float xi_prev = a;
...
int main(int argc, char** argv) {
    ...
    float log_error = log10(error);
    printf("[1>Given_Order:]  Pi=%10.10f  Er=%10.10f  LogEr=%2.2f  \n", integral, error, log_error);
